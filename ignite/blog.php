      ...
      if ( $posts = $wpdb->get_results($query) ) {
        foreach ($posts as $post) { ?>
         <div class="hentry entry xfolkentry archive" id="post-<?php the_ID(); ?>">
            <h3 class="entry-title taggedlink"><a href="<?php echo get_permalink($post->id); ?>" title="Permanent Link to <?php the_title_attribute(); ?>"><?php the_title(); ?></a></h3>
            <ul class="post-info">
               <li class="date"><a href="<?php get_permalink($post->id) ?>" title="<?php the_title_attribute(); ?>"><abbr class="published" title="<?php the_time('Y-M-d'); ?>T<?php the_time('H:i:su'); ?>"><?php the_time('l, F jS, Y \a\t g:i a') ?></abbr></a></li>
              ...
              <p><?php echo (strlen($post->post_content) > 144) ?  strip_tags(substr($post->post_content,0,144)).'... <a href="'.get_permalink($post->id).'">read more</a>' :  strip_tags($post->post_content) ?></p>
            </div>
         </div>
   <?php }
   } ?>
